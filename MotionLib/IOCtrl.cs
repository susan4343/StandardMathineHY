    ...
    public class IOMgr
    {
       ...
       static  object _lock = new object();
        ...
        public void AddCard(string strName, int nIndex, ulong nCardNo)
        {
            ...
            if (flag)
            {
                MessageBox.Show($"{name}卡配置出错","Err",MessageBoxButtons.OK,MessageBoxIcon.Error);
        ...
        bool[] m_bOldInputState;
        bool[] m_bOldOutputState;
        ...
        private void ThreadMonitor()
        {
            ...
            while (!m_bExit)
            {
                ...
                foreach (var tem in m_ioInput)
                {
                    ...
                    if (m_deltgateSystemSingl != null && (tem.Key == "急停" || tem.Key == "暂停" || tem.Key == "启动" || tem.Key == "复位" || tem.Key == "安全门" || tem.Key == "安全光栅"))
                    {
